...
نشانگر `volatile-data-reviewed` داخل `README.md` ماه آخرین بازبینی مدل‌ها، پنجره‌های کانتکست و قیمت‌ها رو نگه می‌داره. گردش‌کار ماهانه فقط بررسی‌های سریع متن و متادیتا رو اجرا می‌کنه و اگه بیش از چهار ماه تقویمی از این نشانگر گذشته باشه، خطا می‌ده؛ ساخت کامل PDF توی اجرای زمان‌بندی‌شده انجام نمی‌شه.
